...
Library is primarly hosted on `Github <https://github.com/MaJerle/lwbtn>`_.

You can get it by:

* Downloading latest release from `releases area <https://github.com/MaJerle/lwbtn/releases>`_ on Github
...
All releases are available on Github `releases area <https://github.com/MaJerle/lwbtn/releases>`_.
...
*CMake* is the main supported build system. Package comes with the ``CMakeLists.txt`` and ``library.cmake`` files, both located in the ``lwbtn`` directory:
...
.. note::
    ...
    File must be renamed to ``lwbtn_opts.h`` first and then copied to the project directory where compiler
...
.. literalinclude:: ../../lwbtn/src/include/lwbtn/lwbtn_opts_template.h
    :language: c
    :linenos:
...
.. note::
    ...
    a global symbol ``LWBTN_IGNORE_USER_OPTS``, visible across entire application.
